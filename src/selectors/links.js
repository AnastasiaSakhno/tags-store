...
const someTagMatches = (tags, regex) =>
  tags.some((tag) => { return regex.test(tag.name) })

const linkMatches = (link, regex) =>
  regex.test(link.name) || regex.test(link.url) || someTagMatches(link.tags, regex)
